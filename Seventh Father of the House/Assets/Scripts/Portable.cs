...
public class Portable : MonoBehaviour
{
    ...
    public virtual void Teleport(Transform fromPortalTransform, Transform toPortalTransform)
    {
        ...
        Vector3 positionOffset = (_parentTransform != null ? _parentTransform.position : transform.position) - fromPortalTransform.position;
        positionOffset = relativeDiff * positionOffset;

        // position
        if(_parentTransform != null)
        {
            _parentTransform.position = toPortalTransform.position + positionOffset;
        }
        else
       ...
       // }
        ...
        if(_parentTransform != null)
        {
            _parentTransform.rotation *= relativeDiff;
        }else
